...
**AI provider:** one provider, called directly via HTTPS/SDK, no agent framework (no LangChain). Recommend Anthropic's Claude API using tool-calling/JSON-schema-constrained output for both the intent-parser and the AI buyer agent — reliable structured output is the load-bearing requirement here, and constrained tool-use output is a good match for "never trust raw text output." OpenAI's structured outputs are an equally valid alternative if you're more familiar with that SDK; the choice does not change the architecture.
