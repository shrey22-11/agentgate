# AgentGate evaluation report - `all` split

Generated 2026-09-03T11:09:33+00:00 - OUR SYSTEM - frozen scenario suite

**114 graded scenarios** (92 deterministic-policy, 22 adversarial natural-language) plus 9 idempotency cases and 12 injected audit-tamper trials.
...
| Structured-parse pass-through (defensive re-validation) | 72.7% |
...
| Decision latency (policy route) p50 / p95 | 65.998 ms / 97.112 ms |
...
|---|---|---|---|
| benign | 42 | 100.0% | 0.0% |
| policy_violating | 50 | 100.0% | 100.0% |
| adversarial (NL) | 22 | 100.0% | 77.3% |

## Adversarial / prompt-injection

22 hostile natural-language messages (fake authority, 'ignore previous instructions', 'pay now', lifted caps, hallucinated products, malformed numbers). Each is parsed defensively (model call stubbed) and its legitimate request is put through the same deterministic policy path.

- Deterministic verdict unchanged by the injection: 100.0%
- Override/manipulation correctly flagged in the audit trail: 100.0%
- Reached a real policy evaluation on resolved fields: 72.7%
- Safely failed closed before policy (unresolvable / malformed): 27.3%
...
|---|--:|--:|--:|--:|--:|--:|
| POST /actions | 92 | 65.998 ms | 97.112 ms | 112.858 ms | 117.148 ms | 69.035 ms |
| POST /ai/actions | 22 | 69.522 ms | 94.607 ms | 95.435 ms | 95.64 ms | 72.576 ms |

## Idempotency under injected duplicates

| Mechanism | Cases | Passed |
|---|--:|--:|
| `duplicate_action` | 4 | 4 |
| `payment_attempt_key` | 2 | 2 |
| `webhook_event` | 3 | 3 |

**`duplicate_action`**

- [ok] `idem-dup-action-counter` - same COUNTER_OFFER request twice: two rows, identical verdict, chain still valid
    - [ok] both HTTP 200
...
- [ok] `idem-dup-action-allow` - same ALLOW request twice: two rows, identical verdict
    - [ok] both HTTP 200
    - [ok] distinct action_request rows
    - [ok] distinct decision rows
    - [ok] identical verdict
...
- [ok] `idem-dup-action-needs-approval` - same NEEDS_APPROVAL request twice: two independent pending decisions
    ...
    - [ok] identical counter-offer
    - [ok] audit chain still valid
- [ok] `idem-dup-action-deny` - same DENY request twice: two rows, identical verdict
    - [ok] both HTTP 200
    - [ok] distinct action_request rows
    - [ok] distinct decision rows
    - [ok] identical verdict
...
- [ok] `idem-payment-key-trailblaze` - a second payment_attempt with the same idempotency key is rejected by the DB
...
- [ok] `idem-webhook-paid` - a replayed payment_link.paid webhook is a no-op (event_id dedupe)
    - [ok] first delivery accepted
    - [ok] replay is duplicate_ignored
    - [ok] exactly one webhook_event row
    - [ok] no 5xx / exception raised
    - [ok] audit chain still valid
- [ok] `idem-webhook-captured` - a replayed payment.captured webhook is a no-op
    - [ok] first delivery accepted
    - [ok] replay is duplicate_ignored
    - [ok] exactly one webhook_event row
    - [ok] no 5xx / exception raised
...
Committed chain after the full run: **yes** valid, 286 events.
