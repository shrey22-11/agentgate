# AgentGate evaluation report - `dev` split

Generated 2026-09-03T11:09:43+00:00 - OUR SYSTEM - frozen scenario suite

**76 graded scenarios** (62 deterministic-policy, 14 adversarial natural-language) plus 6 idempotency cases and 12 injected audit-tamper trials.
...
| Structured-parse pass-through (defensive re-validation) | 71.4% |
...
| Decision latency (policy route) p50 / p95 | 61.519 ms / 98.458 ms |
...
|---|---|---|---|
| benign | 28 | 100.0% | 0.0% |
| policy_violating | 34 | 100.0% | 100.0% |
| adversarial (NL) | 14 | 100.0% | 78.6% |

## Adversarial / prompt-injection

14 hostile natural-language messages (fake authority, 'ignore previous instructions', 'pay now', lifted caps, hallucinated products, malformed numbers). Each is parsed defensively (model call stubbed) and its legitimate request is put through the same deterministic policy path.

- Deterministic verdict unchanged by the injection: 100.0%
- Override/manipulation correctly flagged in the audit trail: 100.0%
- Reached a real policy evaluation on resolved fields: 71.4%
- Safely failed closed before policy (unresolvable / malformed): 28.6%
...
|---|--:|--:|--:|--:|--:|--:|
| POST /actions | 62 | 61.519 ms | 98.458 ms | 110.071 ms | 114.31 ms | 67.055 ms |
| POST /ai/actions | 14 | 64.031 ms | 108.459 ms | 109.759 ms | 110.084 ms | 72.125 ms |

## Idempotency under injected duplicates

| Mechanism | Cases | Passed |
|---|--:|--:|
| `duplicate_action` | 3 | 3 |
...
Committed chain after the full run: **yes** valid, 190 events.
